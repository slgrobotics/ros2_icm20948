...
class MadgwickAHRS:
    ...
    def update(self, gyroscope, accelerometer, magnetometer=None):
        ...
        if magnetometer is None:
            ...
            mx = magnetometer[0]   # assume ENU frame, same as accel/gyro frame (as published by ROS node)
            my = magnetometer[1]
            mz = magnetometer[2]
    ...
    def initialize_from_accel_mag(self, ax, ay, az, mx=None, my=None, mz=None):
        ...
        use_mag = (mx is not None and my is not None and mz is not None)
        if use_mag:
            m = self._normalize3(mx, my, mz)   # assume ENU frame, same as accel/gyro frame (as published by ROS node)
            if m is not None:
                mx, my, mz = m
                # tilt-compensate mag:
